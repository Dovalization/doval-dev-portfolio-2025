...
vi.mock('@/components/desktop-navigation', () => ({
  default: ({ navItems }: { navItems: Array<{ label: string; href: string }> }) => (
...
describe('Header', () => {
  ...
  it('renders social media links with correct hrefs', () => {
    ...
    const githubLink = allLinks.find(link => link.getAttribute('href') === 'https://github.com/test')
    const linkedinLink = allLinks.find(link => link.getAttribute('href') === 'https://linkedin.com/in/test')
