...
Se Naur estiver correto, então cada prática de código limpo é uma estratégia para minimizar essa perda — para manter a teoria do sistema transmissível ao longo do tempo e das equipes. Nomear bem é como você comprime os conceitos da teoria em identificadores, e as revisões de código são como você verifica se a teoria de uma mente foi transferida com sucesso para outra.
...
Jain et al. (ICLR 2024) realizaram um experimento complementar: limpar dados de treinamento renomeando variáveis descritivamente, modularizando funções complexas e inserindo comentários de planejamento em linguagem natural. Resultado: melhoria direta na qualidade de geração de código dos LLMs. Tornar o código mais legível tornou os modelos melhores em trabalhar com ele.
...
Dois estudos de 2025–2026 empurram a convergência ainda mais longe. O EyeLayer (ICPC 2026) alimentou padrões de fixação ocular humana em modelos de sumarização de código e alcançou ganhos de até 13,17% no BLEU-4. O HumanLLM (FSE 2025) aplicou augmentação de trajetória de rastreamento ocular e alcançou +7,16 no CodeBLEU. A atenção humana durante a leitura de código é agora um sinal de treinamento literal — onde programadores olham quando leem código melhora diretamente o que os modelos produzem.
...
Funções bem decompostas têm fluxo de dados mais claro. Tanto humanos quanto LLMs degradam em código profundamente aninhado e de alta complexidade — humanos por causa dos limites da memória de trabalho e modelos porque otimizam localmente sem considerar o contexto global.
...
Knuth disse que programas são obras literárias endereçadas a leitores humanos. O que ele não poderia ter antecipado é que uma nova classe de leitores chegaria para lhe dar razão.
